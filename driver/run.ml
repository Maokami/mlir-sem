...
let print_value (v : Values.mlir_value) : string = Z.to_string v

(* The OCaml runner for the ITree, analogous to vellvm's `step` function. It
   does not use fuel and will loop forever on non-terminating programs. *)
let rec run_tree
    (tree :
      (Obj.t Interp.mlirSemE, Interp.interpreter_state * unit) Interp.itree) :
    Values.mlir_value list option =
