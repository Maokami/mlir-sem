...
let transform_arith_cmp_pred (pred_int : int) : Interp.arith_cmp_pred =
  match pred_int with
  | 0 -> Interp.Eq0
  | 1 -> Interp.Ne
  | 2 -> Interp.Slt
  ...
  | _ ->
      failwith
        ("Unsupported arith.cmpi predicate integer value: "
        ^ string_of_int pred_int)

let get_all_operands (c_op : Bindings.mlir_operation) : string list =
  let num_operands = Bindings.operation_get_num_operands c_op |> Intptr.to_int in
  List.init num_operands (fun i ->
      let operand_val = Bindings.operation_get_operand c_op (Intptr.of_int i) in
      get_value_name operand_val)
...
let rec transform_operation (c_op : Bindings.mlir_operation) : Interp.operation =
  ...
  | "arith.cmpi" ->
      let pred_attr =
        Bindings.operation_get_attribute_by_name c_op
          (Bindings.string_ref_create_from_string "predicate")
      in
      let pred =
        if Bindings.attribute_is_null pred_attr then
          failwith "arith.cmpi operation missing 'predicate' attribute"
        else if Bindings.attribute_is_a_integer pred_attr then
          let pred_int =
            Bindings.integer_attr_get_value_int pred_attr |> Int64.to_int
          in
          transform_arith_cmp_pred pred_int
        else
          failwith "arith.cmpi operation 'predicate' attribute is not an integer"
      in
  ...
  | "cf.br" ->
      ...
      let args = get_all_operands c_op in
      Interp.Term (Cf_Branch (block_name, args))
  | "cf.cond_br" ->
      let true_block =
        Bindings.operation_get_successor c_op (Intptr.of_int 0)
      in
      let true_block_name = get_block_name true_block in
      let num_true_args =
        Bindings.block_get_num_arguments true_block |> Intptr.to_int
      in

      let false_block =
        Bindings.operation_get_successor c_op (Intptr.of_int 1)
      in
      ...
      let rec split_list at list =
        if at = 0 then ([], list)
          ...
          | [] -> ([], [])
          | h :: t ->
              let l1, l2 = split_list (at - 1) t in
              (h :: l1, l2)
      in

      let true_args, false_args = split_list num_true_args arg_operands in

      Interp.Term
        (Cf_CondBranch
           (cond_name, true_block_name, true_args, false_block_name, false_args))
...
and transform_block (c_block : Bindings.mlir_block) : Interp.block =
  ...
  let num_args = Bindings.block_get_num_arguments c_block |> Intptr.to_int in
  let block_args =
    List.init num_args (fun i ->
        let arg_val = Bindings.block_get_argument c_block (Intptr.of_int i) in
        let arg_name = get_value_name arg_val in
        let arg_type = Bindings.value_get_type arg_val in
        (arg_name, transform_mlir_type arg_type))
  in
  let first_op = Bindings.block_get_first_operation c_block in
  let ops = transform_operations_in_block first_op in
  {Interp.block_name; Interp.block_args; Interp.block_ops = ops}
